...
out_path = r"c:\Users\higashi\Projects\039_sales_system\extract_info.txt"
...
try:
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(str(df_prod.columns.tolist()[:15]) + "\n\n")
        f.write("Order Columns:\n")
        f.write(str(df_order.columns.tolist()[:17]) + "\n\n")
        
        f.write("Product Sample:\n")
        for row in df_prod.head(2).to_dict('records'):
            f.write(str({k: row[k] for (i, k) in enumerate(row) if i < 15}) + "\n")
