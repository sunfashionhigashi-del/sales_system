...
- Status policy: rows with invoice numbers import as `請求済`, locked and archived. Rows without invoice numbers are judged by EXW; EXW dates on or before the import day are archived as shipped history while remaining `未請求`. Non-shipped rows remain active.
...
- Imported 2,800 Mobiron rows into Supabase. Verification counts: archived 2,598, locked 565, suppliers `日清紡` 2,394 and `八木熊` 406, currencies JPY 2,397 and USD 403.
